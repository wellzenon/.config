...
awesome.connect_signal("startup", function(args)
	awful.util.spawn(
		'bash -c "xrandr --output eDP-1 --mode 1920x1080 --rate 120.00 --primary --output HDMI-1-0 --right-of eDP-1 --mode 1440x900 --rate 60.00"'
	)
end)
...
clientkeys = mytable.join(
	...
	--awful.key({ modkey, "Shift"   }, "c",      function () os.execute(string.format([[wmctrl -l | while read -r line do wmctrl -c `echo "$line" | sed 's/.*  [0-9]* [your_hostname] //'` done]], 'run')) end
